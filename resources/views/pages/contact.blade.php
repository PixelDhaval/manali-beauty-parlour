@php
    ...
    $faqs = [
        [
            ...
            'answer' =>
                'Yes, we welcome walk-ins based on availability, though booking ahead guarantees your preferred time slot and beautician.',
        ],
        [
            'question' => 'Do I need to book in advance?',
            'answer' =>
                'We recommend booking at least a day in advance, especially for facials, bridal beauty and package treatments.',
        ],
        [
            'question' => 'Is parking available?',
            'answer' => 'Yes, convenient parking is available near the salon for our clients.',
        ],
        [
            'question' => 'What payment methods are accepted?',
            'answer' => 'We accept cash, all major cards and UPI payments for your convenience.',
        ],
        [
            'question' => 'What is the cancellation policy?',
            'answer' =>
                'We kindly request at least 4 hours notice for cancellations or rescheduling so we can accommodate other clients.',
        ],
    ];
@endphp
...
<x-layouts.app title="Contact Us"
    ...
    <x-layout.page-hero eyebrow="Contact Us" heading="Let's Make Your Beauty Journey Special"
        description="Reach out to us or book your appointment online &mdash; we'd love to welcome you." />

    @if (session('success'))
        <div class="px-5 pt-10 sm:px-8">
            <div
                class="mx-auto flex max-w-7xl items-center gap-3 rounded-2xl border border-herbal-300 bg-herbal-50 px-5 py-4 text-sm font-medium text-herbal-700">
    ...
    <section class="px-5 py-16 sm:px-8 sm:py-20">
        <div class="mx-auto grid max-w-7xl gap-10 lg:grid-cols-5">
            ...
            <div class="flex flex-col gap-6 lg:col-span-2" data-reveal>
                ...
                <div class="flex flex-col gap-5">
                    <div class="flex items-start gap-4 rounded-2xl border border-primary-100 bg-white p-5">
                        <span
                            class="flex h-11 w-11 flex-none items-center justify-center rounded-xl bg-primary-50 text-primary-600">
                        ...
                        <div>
                            ...
                            <a href="tel:{{ $business['phone_link'] }}"
                                class="block text-sm text-ink-600 hover:text-primary-700">{{ $business['phone'] }}</a>
                            <a href="{{ $business['whatsapp_link'] }}" target="_blank" rel="noopener"
                                class="block text-sm text-ink-600 hover:text-primary-700">Chat on WhatsApp</a>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-4 rounded-2xl border border-primary-100 bg-white p-5">
                        ...
                        <div>
                            ...
                            <a href="{{ $business['social']['instagram'] }}" target="_blank" rel="noopener"
                                class="block text-sm text-ink-600 hover:text-primary-700">Instagram</a>
                            @if (isset($business['social']['facebook']))
                                <a href="{{ $business['social']['facebook'] }}" target="_blank" rel="noopener"
                                    class="block text-sm text-ink-600 hover:text-primary-700">Facebook</a>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            {{-- Appointment Form --}}
            <div class="rounded-3xl border border-primary-100 bg-white p-6 sm:p-8 lg:col-span-3" data-reveal
                style="--reveal-delay: 100ms">
                <x-ui.eyebrow>Book Appointment</x-ui.eyebrow>
                <h2 class="mt-3 font-serif text-2xl font-semibold text-ink-900 sm:text-3xl">Request Your Appointment
                </h2>
                <p class="mt-2 text-sm text-ink-600">Fill in your details and our team will confirm your slot shortly.
                </p>
                ...
                <form method="POST" action="{{ route('appointments.store') }}"
                    ...
                    <div class="flex flex-col gap-1.5 sm:col-span-2">
                        <label for="email" class="text-sm font-medium text-ink-800">Email <span
                                class="font-normal text-ink-400">(optional)</span></label>
                    ...
                    <div class="flex flex-col gap-1.5 sm:col-span-2">
                        <label for="preferred_service" class="text-sm font-medium text-ink-800">Preferred
                            Service</label>
                        <select id="preferred_service" name="preferred_service" required
                            class="rounded-xl border border-primary-100 bg-cream/40 px-4 py-3 text-sm text-ink-900 outline-none transition focus:border-primary-400 focus:bg-white focus:ring-2 focus:ring-primary-100">
                            <option value="" disabled {{ old('preferred_service') ? '' : 'selected' }}>Select a
                                service</option>
                            @foreach ($services as $service)
                                <option value="{{ $service['name'] }}"
                                    {{ old('preferred_service') === $service['name'] ? 'selected' : '' }}>
                                    {{ $service['name'] }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex flex-col gap-1.5 sm:col-span-1">
                        ...
                        <select id="preferred_time" name="preferred_time" required
                            class="rounded-xl border border-primary-100 bg-cream/40 px-4 py-3 text-sm text-ink-900 outline-none transition focus:border-primary-400 focus:bg-white focus:ring-2 focus:ring-primary-100">
                            <option value="" disabled {{ old('preferred_time') ? '' : 'selected' }}>Select a
                                time</option>
                            @foreach ($timeSlots as $slot)
                                <option value="{{ $slot }}"
                                    {{ old('preferred_time') === $slot ? 'selected' : '' }}>{{ $slot }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex flex-col gap-1.5 sm:col-span-2">
                        <label for="message" class="text-sm font-medium text-ink-800">Message <span
                                class="font-normal text-ink-400">(optional)</span></label>
                    ...
                    <div class="sm:col-span-2">
                        <x-ui.button type="submit" icon="arrow-right" class="w-full sm:w-auto">Book
                            Appointment</x-ui.button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <section class="px-5 pb-20 sm:px-8 sm:pb-28">
        <div class="mx-auto max-w-7xl overflow-hidden rounded-3xl border border-primary-100" data-reveal>
            <iframe src="{{ $business['map_embed_url'] }}" class="h-96 w-full" style="border:0;" allowfullscreen
                loading="lazy" referrerpolicy="no-referrer-when-downgrade"
                title="Manali Herbal Beauty Parlour location"></iframe>
        </div>
    </section>
    ...
    <section class="px-5 py-20 sm:px-8 sm:pb-28">
        <div class="mx-auto max-w-7xl">
            <x-ui.cta-banner heading="Book Your Beauty Appointment Today"
                description="Our team is ready to welcome you for a personalised herbal beauty experience." />
        </div>
    </section>
</x-layouts.app>
