...
<footer class="border-t border-primary-100 bg-white">
    <div class="mx-auto max-w-7xl px-5 py-16 sm:px-8">
            ...
            <div class="flex flex-col gap-4 lg:col-span-1">
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ $business['social']['instagram'] }}" target="_blank" rel="noopener"
                        class="flex h-9 w-9 items-center justify-center rounded-full bg-primary-50 text-primary-600 transition hover:bg-primary-600 hover:text-white"
                ...
                <ul class="flex flex-col gap-2.5 text-sm text-ink-600">
                    @foreach ($quickLinks as $link)
                        <li><a href="{{ route($link['route']) }}"
                                class="hover:text-primary-700">{{ $link['label'] }}</a></li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="flex flex-col gap-4">
                ...
                <ul class="flex flex-col gap-2.5 text-sm text-ink-600">
                    @foreach ($serviceLinks as $service)
                        <li><a href="{{ route('services') }}" class="hover:text-primary-700">{{ $service }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
        ...
        <div
            class="mt-12 flex flex-col items-center justify-between gap-4 border-t border-primary-100 pt-8 text-xs text-ink-400 sm:flex-row">
